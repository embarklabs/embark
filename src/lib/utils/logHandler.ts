...
class LogHandler {
   ...
   */
  constructor({events, logger, processName, silent}: any) {
    this.events = events;
    this.logger = logger;
   ...
   * @returns {void}
   */
  public handleLog(msg: any, alreadyLogged = false) {
    if (!msg) {
      return;
    }
    ...
    if (typeof msg.message === "string") {
      ...
      msg.message.forEach((message: any) => {
        if (Array.isArray(message)) {
          return message.forEach((line: string) => processedMessages.push(line));
        }
        if (typeof message === "object") {
          return processedMessages.push(JSON.stringify(msg.message));
        }
        message.toString().split("\n").forEach((line: string) => processedMessages.push(line));
      });
