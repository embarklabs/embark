...
class AccountParser {
  public static parseAccountsConfig(accountsConfig: any, web3: any, logger: any, nodeAccounts: any) {
    let accounts: any = [];
    if (accountsConfig && accountsConfig.length) {
      accountsConfig.forEach((accountConfig: any) => {
  ...
  public static getAccount(accountConfig: any, web3: any, logger = console, nodeAccounts: any) {
    const {utils} = require("web3");
    const returnAddress = web3 === false;
    let hexBalance: any = null;
    ...
    if (accountConfig.nodeAccounts) {
      ...
      return nodeAccounts.map((account: any) => {
    ...
    if (accountConfig.privateKeyFile) {
      const privateKeyFile: string = path.resolve(fs.dappPath(), accountConfig.privateKeyFile);
      let fileContent: any = fs.readFileSync(privateKeyFile).toString();
      ...
      return fileContent.map((key: string, index: any) => {
