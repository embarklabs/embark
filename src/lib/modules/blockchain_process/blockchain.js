...
Blockchain.prototype.setupProxy = async function () {
  ...
  if (this.config.wsRPC) {
    wsProxy = proxy.serve(this.proxyIpc, this.config.wsHost, this.config.wsPort, true, this.config.wsOrigins, addresses);
  }

  [this.rpcProxy, this.wsProxy] = await Promise.all([proxy.serve(this.proxyIpc, this.config.rpcHost, this.config.rpcPort, false, null, addresses), wsProxy]);
};
