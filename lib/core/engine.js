...
class Engine {
  ...
  init(_options) {
    ...
    this.events = this.events || options.events || this.events || new Events();
    this.logger = this.logger || options.logger || new Logger({logLevel: options.logLevel || this.logLevel || 'debug', events: this.events, logFile: this.logFile});
