  ...
  web3Service(options) {
    ...
    if (this.web3 === undefined) {
      ...
      if (this.config.contractsConfig.deployment.type === "rpc") {
        web3Endpoint = 'http://' + this.config.contractsConfig.deployment.host + ':' + this.config.contractsConfig.deployment.port;
        providerOptions = {
          web3: this.web3,
    ...
    async.waterfall([
      ...
      self.servicesMonitor.addCheck(SERVICE, function (cb) {
        ...
        async.waterfall([
          ...
          if (msg === NO_NODE) {
            ...
            self.servicesMonitor.stopCheck(SERVICE);
            provider.stop();
            self.web3 = null;
            ...
            setTimeout(() => {
              ...
              self.web3Service(options);
            }, 2000);
            return;
            // Restart provider
            // provider = new Provider(providerOptions);
            // provider.startWeb3Provider((err) => {
            //   console.log('I RESTARTED!!!');
            //   if (err) {
            //     self.logger.error('Error while restarting the provider');
            //     self.logger.error(err);
            //   }
            //   return cb(noNodeObj);
            // });
          }
          // if (self.isRunningBlockchain && !self.alreadyCalledReady) {
          //   self.alreadyCalledReady = true;
          //   self.events.emit('file-event', 'contract', '*');
          // }
          cb(msg);
        });
      });
