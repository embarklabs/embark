...
Config.prototype.loadContractsConfigFile = function() {
  ...
  if (newContractsConfig.gas != "auto") {
      newContractsConfig.gas = utils.getWeiBalanceFromString(newContractsConfig.gas, web3);
  }
  if ('deployment' in newContractsConfig) {
    if ('accounts' in newContractsConfig.deployment) {
      for (var i = 0; i < newContractsConfig.deployment.accounts.length; i++) {
        newContractsConfig.deployment.accounts[i].balance = utils.getWeiBalanceFromString(newContractsConfig.deployment.accounts[i].balance, web3);
      }
    }
  }
  for (var contractName in newContractsConfig.contracts) {
    if (newContractsConfig.contracts[contractName].gas != "auto") {
      if (newContractsConfig.contracts[contractName].gas) {
        newContractsConfig.contracts[contractName].gas = utils.getWeiBalanceFromString(newContractsConfig.contracts[contractName].gas, web3);
      }
    }
    if (newContractsConfig.contracts[contractName].gasPrice != "auto") {
      if (newContractsConfig.contracts[contractName].gasPrice) {
        newContractsConfig.contracts[contractName].gasPrice = utils.getWeiBalanceFromString(newContractsConfig.contracts[contractName].gasPrice, web3);
      }
    }
  }
