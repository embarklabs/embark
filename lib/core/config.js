...
class Config {
  ...
  constructor(logger, events, context, embarkConfigFile, env, configDir, chainsFile, autoLoadAllConfigs, contractsConfig) {
    this.logger = logger;
    this.events = events;
    ...
    this._autoLoadConfigs = autoLoadAllConfigs === undefined || autoLoadAllConfigs;
    if(this._autoLoadConfigs){
  ...
  loadContractsConfigFile() {
    ...
    let newContractsConfig;
    //if(this._autoLoadConfigs){
      let configFilePath = this._getFileOrOject(this.configDir, 'contracts.json', 'contracts');
      newContractsConfig = this._mergeConfig(configFilePath, configObject, this.env);
    //}else{
      //newContractsConfig = utils.recursiveMerge(configObject, this.contractsConfig);
      //newContractsConfig = utils.recursiveMerge(configObject['default'] || {}, configObject[this.env]);
    //}
...
inversify.decorate(inversify.inject('contractsConfig'), Config, 8);
inversify.decorate(inversify.optional(), Config, 8);
