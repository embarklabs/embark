...
class Provider {
  constructor(options) {
    ...
    this.ethereumNetworkID = options.ethereumNetworkID;
  ...
  startWeb3Provider(callback) {
    ...
    if (this.type === 'rpc') {
      ...
      getTransport.on('error', e => self.logger.error('Ledger wallet connection Error', e));
      getTransport.on('end', e => self.logger.error('Ledger wallet connection ended', e));
    } else {
