...
class IPFS {
  ...
  constructor(embark, options) {
    ...
    this.protocol = options.protocol || this.storageConfig.protocol;
  ...
  setServiceCheck() {
    ...
    self.addCheck('IPFS', function (cb) {
      ...
      let callback = function(err, body){
        if (err) {
      ...
      let port = self.port == '80' ? '' : ':' + self.port;
      let url = self.protocol + '://' + self.host + port + '/api/v0/version';
      if(self.protocol === 'https') utils.httpsGetJson(url, callback);
      else utils.httpGetJson(url, callback);
    });
  }
