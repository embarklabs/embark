...
function getFilesFromDir(filePath, cb) {
  fs.readdir(filePath, (err, files) => {
    ...
    const testFiles = files.map(file => path.join(filePath, file));
    cb(null, testFiles);
  });
}
...
function runSolidityTests(files, options, cb) {
  const loglevel = options.loglevel || 'warn';
  // read all the files and insert remix_tests.sol
  ...
  async.waterfall([
    ...
    results.forEach((result) => {
      totalPass = totalPass + result.passingNum;
      totalFailures = totalFailures + result.failureNum;
    });
    cb(null, {solcTest: {failures: totalFailures}, name: 'solcTest'});
  });
...
module.exports = {
  run: function (options) {
    ...
    async.waterfall([
      ...
      let totalFailures = results.reduce((acc, result) => acc + result[result.name].failures, 0);
      if (totalFailures) {
