...
class Pipeline {
  ...
  constructor(config, events, logger, plugins) {
    this.buildDir = config.buildDir;
    this.contractsFiles = config.contractsFiles;
    this.assetFiles = config.assetFiles;
    this.events = events;
    this.logger = logger;
  ...
  init() {
    this.events.on('code-generator-ready', () => {
      this.events.request('code', function (abi, contractsJSON) {
        this.currentAbi = abi;
        this.contractsJSON = contractsJSON;
        this.build(abi, contractsJSON, null, () => {
          if (this.watch) {
            this.watch.restart(); // Necessary because changing a file while it is writing can stop it from being watched
          }
...
inversify.decorate(inversify.injectable(), Pipeline);
inversify.decorate(inversify.inject(Config), Pipeline, 0);
inversify.decorate(inversify.inject(Events), Pipeline, 1);
inversify.decorate(inversify.inject(Logger), Pipeline, 2);
inversify.decorate(inversify.inject(Plugins), Pipeline, 3);
