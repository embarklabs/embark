  ...
  deployContracts(compileOnlyIfNeeded, done) {
    ...
    async.waterfall([
      function buildContracts(callback) {
        if (compileOnlyIfNeeded) {
          if (self.contractsManager.compiledContracts && Object.keys(self.contractsManager.compiledContracts).length) {
            // Contracts have already been compiled
            return callback();
          }
        }
