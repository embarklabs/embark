...
class DeployManager {
  constructor(events, logger, config, plugins, ethereumProvider, contractsManager, trackContracts, onlyCompile) {
    this.config = config;
    this.logger = logger;
    ...
    this.web3 = ethereumProvider.providerObject; // TODO: change this.web3 to this.ethProvider, which should be of type IEthereumProvider
    this.chainConfig = (trackContracts !== false) ? this.config.chainTracker : false;
    this.contractsManager = contractsManager;
    this.gasLimit = false;
    this.fatalErrors = false;
    this.deployOnlyOnConfig = false;
    this.onlyCompile = onlyCompile !== undefined ? onlyCompile : false;
  }
...
inversify.decorate(inversify.injectable(), DeployManager);
inversify.decorate(inversify.inject(Events), DeployManager, 0);
inversify.decorate(inversify.inject(Logger), DeployManager, 1);
inversify.decorate(inversify.inject(Config), DeployManager, 2);
inversify.decorate(inversify.inject(Plugins), DeployManager, 3);
inversify.decorate(inversify.inject('EthereumProvider'), DeployManager, 4);
inversify.decorate(inversify.inject(ContractsManager), DeployManager, 5);
inversify.decorate(inversify.inject('trackContracts'), DeployManager, 6);
inversify.decorate(inversify.optional(), DeployManager, 6);
inversify.decorate(inversify.inject('onlyCompile'), DeployManager, 7);
inversify.decorate(inversify.optional(), DeployManager, 7);
