...
class CodeGenerator {
  ...
  generateStorageInitialization(useEmbarkJS) {
    ...
    if (self.storageConfig.provider === 'ipfs' && self.storageConfig.enabled === true) {
      let block = "\nEmbarkJS.Storage.setProvider('" + self.storageConfig.provider + "', {server: '" + self.storageConfig.host + "', port: '" + self.storageConfig.port + "', protocol: '" + self.storageConfig.protocol + "', getUrl: '" + self.storageConfig.getUrl + "'});";
