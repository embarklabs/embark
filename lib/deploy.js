...
Deploy.prototype.generate_info_file = function() {
  var result = "";
  ...
  for(className in this.contractDB) {
    var isCompiled = this.contractDB[className].compiled

    contracts[className] = {
      abi: isCompiled ? this.contractDB[className].compiled.info.abiDefinition : null
      ,address:this.deployedContracts[className]
      ,code: isCompiled ? '0x'+this.contractDB[className].compiled.code : null
      ,runtimeBytecode: isCompiled ? '0x'+this.contractDB[className].compiled.runtimeBytecode : null
    }
  }

  result += 'contracts = '+JSON.stringify(contracts)+';';

  return result;
};

Deploy.prototype.generate_and_write_info_file = function(destFile) {
  var result = this.generate_info_file();
