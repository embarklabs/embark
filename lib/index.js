...
Embark = {
  ...
  deployContracts: function(env, contractFiles, destFile, chainFile, withProvider, withChain, cb) {
    ...
    var deploy = new Deploy(env, contractFiles, this.blockchainConfig.config(env), this.contractsConfig, this.chainManager, withProvider, withChain, web3);
    ...
    deploy.deploy_contracts(env, function() {
      ...
      deploy.generate_abi_file(function(abi){
        result += abi;
        cb(result);
      });
