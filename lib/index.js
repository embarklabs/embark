      ...
      function checkStorageService(callback){
        ...
        checkFn.fn(function (serviceCheckResult) {
          if (!serviceCheckResult.status || serviceCheckResult.status === 'off') {
            return callback({message: __('Cannot upload: {{platform}} node is not running on {{url}}.', {platform: platform, url: `http://${engine.config.storageConfig.host}:${engine.config.storageConfig.port}`})});
          }
          callback();
        });
      },
...
inversify.decorate(inversify.injectable(), Embark);
inversify.decorate(inversify.inject(Events), Embark, 0);
inversify.decorate(inversify.inject(Logger), Embark, 1);
inversify.decorate(inversify.inject('options'), Embark, 2);
inversify.decorate(inversify.optional(), Embark, 2);
